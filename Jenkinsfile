  ...
  stages {
    ...
    stage('plan') {
            steps {
                withAWS(credentials:'8f8055f0-fef5-47b6-915b-d34669729c37') {
                sh  """
                    ${TERRAFORM_CMD} plan -var "aws['aws_access_key']=${AWS_ACCESS_KEY_ID}" -var "aws['aws_secret_key']=${AWS_SECRET_KEY_ID}" -var "aws['account']=${AWS_ACCOUNT_NUM}" -var "key_pair=${AWS_PEM_KEY}" -out=${WORK_DIR}tfoutput.plan -input=false
                    """
                }
            }
        }
